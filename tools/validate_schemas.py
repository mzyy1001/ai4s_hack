...
def check_evidence_specificity(rep, report_path):
    """检查 evidence_refs 是否逐条特异（契约 §1.4.1）。

    为什么需要机器检查：霰弹式引用**形式上完全合规** —— 非空、每条都能解析到
    registry —— schema 校验一路绿灯，但可审计性归零。实测出现过一份报告里
    每条 finding 都挂了全部 24 条引用，读者无法回答「哪条外部记录支撑了哪条判断」。
    只有统计「引用集合的重复度」才能把它抓出来。
    """
    import collections
    with open(report_path, encoding="utf-8") as fh:
        r = json.load(fh)
    fs = r.get("all_findings") or []
    if not fs:
        return

    sigs = collections.Counter(
        tuple(sorted(f.get("evidence_refs") or [])) for f in fs)
    top_sig, top_n = sigs.most_common(1)[0]
    share = top_n / len(fs)
    tag = os.path.basename(os.path.dirname(os.path.abspath(report_path))) or os.path.basename(report_path)
    label = f"evidence_refs 特异性 {tag}"

    shotgun = share > 0.5 and len(top_sig) > 1
    rep.check(not shotgun, label,
              (f"{top_n}/{len(fs)} 条 finding（{share:.0%}）共享完全相同的 "
               f"{len(top_sig)} 条引用列表 —— 霰弹式引用，无法追溯"
               f"哪条证据支撑哪条判断（契约 §1.4.1）") if shotgun
              else f"最大重复组 {top_n}/{len(fs)}（{share:.0%}）")
...
def main():
    ...
    # 传入 review_report.json 路径时，只跑报告层的可审计性检查
    reports = [a for a in sys.argv[1:] if a.endswith(".json") and os.path.isfile(a)]
    if reports:
        for rp in reports:
            check_evidence_specificity(rep, rp)
        print("\n" + "=" * 60)
        if rep.failures:
