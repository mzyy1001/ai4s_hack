...
# --------------------------------------------------------------- 复检既有语料
#
# 为什么需要这个：把关逻辑是**逐步补强**的（`_in_culture_context` 就是踩了
# CCL4 的坑之后补的）。但补强只影响**此后新生成**的条目 —— 早先生成、
# 已经躺在 datasets/ 里的假题不会自己消失，也不会有人发现，
# 直到它悄悄污染一轮 A/B 评分。
#
# 实际发生过：PMC11985696 是在 `_in_culture_context` 之前生成的，
# 标准答案写「细胞系 CCL4 误认」，而该文里 CCL4 是趋化因子基因
# （与 IL1B / MMP9 / CCL3 / CCL8 并列的 iCAF 标志物），根本没用这个细胞系。
# 用当前把关重跑，该条会被正确拒绝。
#
# 所以：**每次改把关逻辑之后都要 `--revalidate` 一遍**，
# 让旧条目也接受新标准的审查。

def revalidate(root, purge=False):
    """用当前把关逻辑复检已生成的语料，报出（可选删除）不再成立的条目。"""
    root = os.path.abspath(root)
    if not os.path.isdir(root):
        print(f"语料目录不存在：{root}\n"
              f"（datasets/ 不入库，先跑一次生成，或从 dev 分支历史取回）")
        return 1

    bad, ok, skipped = [], 0, 0
    for pmcid in sorted(os.listdir(root)):
        d = os.path.join(root, pmcid)
        meta_p = os.path.join(d, "meta.json")
        if not os.path.isfile(meta_p):
            continue
        meta = json.load(open(meta_p, encoding="utf-8"))
        if meta.get("kind") != "cell_line":
            skipped += 1
            continue                      # 目前只有 cell_line 有上下文歧义风险

        gt = meta.get("ground_truth", {})
        name = gt.get("cell_line")
        xml_p = os.path.join(d, "fulltext.xml")
        if not (name and os.path.isfile(xml_p)):
            skipped += 1
            continue

        text = body_text(open(xml_p, encoding="utf-8").read())
        if _in_culture_context(text, name):
            ok += 1
            continue
        bad.append((pmcid, name, gt.get("cellosaurus_accession")))

    print(f"复检 cell_line 语料：通过 {ok}，可疑 {len(bad)}，跳过 {skipped}")
    for pmcid, name, acc in bad:
        print(f"  ✗ {pmcid}：标准答案称用了细胞系 {name}（{acc}），"
              f"但全文里 {name} 从未出现在细胞培养语境中 —— "
              f"极可能是与基因符号重名导致的假题")
    if bad and purge:
        for pmcid, _, _ in bad:
            shutil.rmtree(os.path.join(root, pmcid), ignore_errors=True)
            print(f"  已删除 {pmcid}")
        _rewrite_index(root)
    elif bad:
        print("\n加 --purge 可直接删除这些条目并重建 index.json。")
    return 1 if bad and not purge else 0


def _rewrite_index(root):
    papers = []
    for pmcid in sorted(os.listdir(root)):
        p = os.path.join(root, pmcid, "meta.json")
        if os.path.isfile(p):
            papers.append(json.load(open(p, encoding="utf-8")))
    by_kind = {}
    for m in papers:
        by_kind.setdefault(m.get("kind", "unknown"), []).append(m["pmcid"])
    idx_p = os.path.join(root, "index.json")
    idx = json.load(open(idx_p, encoding="utf-8")) if os.path.isfile(idx_p) else {}
    idx["papers"] = papers
    idx["by_kind"] = {k: sorted(v) for k, v in sorted(by_kind.items())}
    with open(idx_p, "w", encoding="utf-8") as fh:
        json.dump(idx, fh, ensure_ascii=False, indent=2)
    print(f"  index.json 已重建：{len(papers)} 篇")
...
def main():
    ...
    ap.add_argument("--revalidate", action="store_true",
                    help="用当前把关逻辑复检已生成的语料，报出不再成立的假题。"
                         "**每次改把关逻辑后都应跑一次** —— 补强只作用于新生成的条目，"
                         "旧假题不会自己消失")
    ap.add_argument("--purge", action="store_true",
                    help="配合 --revalidate：直接删除可疑条目并重建 index.json")
    a = ap.parse_args()

    if a.revalidate:
        return revalidate(a.out, purge=a.purge)
