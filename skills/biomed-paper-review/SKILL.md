...
description: 生物医药论文结构化抽取与审稿辅助。输入一篇论文（PDF / JATS XML / 纯文本），先抽出可复用的结构化事实表 —— 研究设计、受试对象、干预与分组、测量与终点、关键数值、主张清单与各类声明，每个字段都带状态与原文出处，并给出字段解析率；同时解析图表，从原图读取实验条件与关键数值并定位回图号、面板与页码。随后从六个维度审阅：宏观逻辑与报告完整性、实验方法与可复现性、统计方法与样本量、图表呈现规范、伦理合规与动物福利、结论与证据的匹配度。判断不止于阅读 —— 还用确定性计算复算稿件内的数值与统计关系，并对照公开权威数据库核验参考文献、试验注册、细胞系与各类标识符。输出结构化结果表、可回溯到原文位置的分级审核发现、图表与表格记录、抽取覆盖率与复核置信度，以及分优先级的人工复核建议；工具失败、外部源不可达或材料不可得时登记为系统限制，不归责稿件。支持完整审核 / 仅结构化抽取 / 图表解析 / 定向核查四种模式。当用户需要预审或复核生物医药论文与稿件、把论文抽成可复用的结构化数据、读取图表中的关键数值、核验参考文献与标识符、检查统计方法与伦理合规、或判断结论是否超出证据支撑时使用。
---
