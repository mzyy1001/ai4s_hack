...
```
主会话（你）
├── task(general) → 发现子会话 A      全文 + 科学效度指令（开放式，无清单）
├── task(general) → 发现子会话 B      全文 + 报告规范指令（系统清单）
│      └── 主会话取 A ∪ B 去重后路由   **两条都不给规则库**
...
## 1. Layer 1 · 发现层（两条并行通道）

```
task(general, description="Discovery-A 科学效度")   全文 + A 指令
task(general, description="Discovery-B 报告规范")   全文 + B 指令
        ↓
   主会话取 A ∪ B，去重后再路由
```

**为什么必须两条通道**：实测教训 —— 单通道发现层挂着一张「报告规范核查清单」，
模型就**严格按清单办事**：真实论文一轮跑出 39 条候选，几乎一一对应清单条目，
而裸模型在同一篇论文上找出的最强两条（敲入等位基因同时敲除全长 RAGE，
使所谓「独立遗传学验证」自带混杂；单一 7 dpi 时间点撑不起「加速再生」）
**一条都没进候选池**。下游各层忠实地把这份贫瘠的候选集传导到底。
清单本意是下限，实际成了上限。**因此把「像科学家一样质疑」与「照清单核查」
拆成两个互不干扰的通道，各自拿全文，各自开跑。**

两条通道的 prompt 里都**只放**：论文全文、图表清单、该通道指令。
**不要**放契约、schema、M2–M7 规则库 —— 那会让结构化取代发现。

### 1.A 通道 A · 全局科学效度发现（开放式，不给清单）

> 你是该领域资深同行评审。通读全文，然后问自己：**这项研究的设计，
> 到底能不能支撑它宣称的因果/机制结论？** 把你的质疑全部列出来。
>
> **这里没有清单，也不要去凑清单。** 按下面这些角度自由深挖，
> 想到清单之外的问题更好 —— 那正是本通道的价值：
>
> - 研究设计本身能否支撑主因果/机制主张？还是只能支撑相关性？
> - 有无混杂因素或**同样说得通的替代解释**？（含处理带来的副效应，
>   例如代谢/体重/血糖改变本身即可解释结局）
> - 对照是否恰当且**匹配**？遗传背景、品系、窝别、性别、年龄、批次是否可比？
>   有无必需而缺失的对照（空载体、假手术、阳性对照、同窝对照）？
> - **实验构件本身是否自带混杂？** 转基因/敲入/敲除等位基因、载体、
>   报告基因、给药方式是否同时扰动了别的东西，使归因不唯一？
>   （读构件示意图与克隆描述，不要只读正文结论）
> - **真正的实验单元/分析单元是什么？** 有无伪重复
>   （把细胞、纤维、切片、视野、重复测量当作独立样本）？
> - 所测**终点**是否真的度量了它声称的过程？替代终点与目标结局的距离有多远？
> - **时序**是否支撑「加速 / 挽救 / 预防 / 恢复 / 逆转」这类动力学措辞？
>   单一时间点能否区分「更快」与「不同步 / 程度不同 / 取材偏倚」？
> - 干预组与对照组在因果上是否可比？分配、饲养、操作是否引入系统差异？
> - 阴性结果是否被当成「无效应/等效」的证据？（缺证据 ≠ 无效应的证据）
> - **构件设计、Methods、图与图注、Results、因果主张**五者之间有无矛盾？
>   尤其：图注/示意图里写明的构件性质，是否与正文的归因相冲突？
>
> **优先召回**：拿不准的也列，标 `confidence: low`；定位不到就把 `locations` 留空。
> 不需要给严重度，不需要套 schema。每条要落到具体位置与具体内容，不要泛泛而谈。

### 1.B 通道 B · 系统性报告规范发现（保留原清单）

> 你是资深同行评审，本轮**专做报告规范与内部一致性的系统核查**
> （科学效度由另一条通道负责，你不必操心）。逐项扫，扫全：
>
> 表格分母与计数是否自洽；同一结局在 Abstract/Results/Discussion/图注之间
> 数值、P 值、方向、组名是否一致；结论是否超出数据；纳入标准与基线表是否矛盾；
> 盲法与随机化是否报告；安全性结论与随访时长是否匹配；样本量有无依据与功效分析；
> 精确 n 与「n≥」混用、分母不清；统计检验与设计是否匹配；多重比较有无校正；
> 正态性/方差齐性前提；伦理、注册、知情同意、IACUC/ARRIVE 要素是否齐全；
> 命名与基因符号书写是否规范一致；图注与正文（面板字母、n、检验）是否对应；
> 试剂标识符（货号/克隆号/RRID）；数据与材料可用性声明是否充分。
>
> 同样**优先召回**，标注方式同通道 A。
...
- 逐条比对，**同一问题只保留一条**，合并时保留更具体的那条描述与定位；
- 合并记录写进 `candidate_resolution_log`（`merged`），**不得静默丢弃**；
- 只有同时看两个 section 才能发现的矛盾，`suggested_route` 记 `["reconcile"]`；
- 通道 A 的候选若涉及设计效度/混杂/分析单元，优先路由 `methods` 与 `statistics`，
  跨构件与主张的矛盾路由 `reconcile`。

合并后的候选集形状与原来一致，下游路由、专家、工具、校正、契约**一律不变**：
