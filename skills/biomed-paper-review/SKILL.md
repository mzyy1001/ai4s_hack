...
但**正文不能跟着一起切**。实测代价（同一篇 PLOS ONE 真实论文，同模型同全文）：

| | 裸模型（全文 + 开放提问） | 旧架构（发现层压成候选 → 专家只拿证据包） |
| --- | --- | --- |
| 发现数 | **63**（critical 2） | 43（critical 0） |
| 最深的两条 | 敲入等位基因同时是全长 RAGE-null，所谓「独立遗传学验证」自带混杂；单一 7 dpi 撑不起「加速再生」 | **一条都没有** |

原因是**两次压缩、且第二次由第一次决定**：发现层把论文压成候选清单，
证据包又按候选清单去正文里取片段 —— 于是没人标注过的段落（构件示意图、克隆描述、
品系来源）没有任何一层读得到，漏了就结构性不可恢复。
而且「隔离」本就是假的：统计包已经是全文的 71%，主张包 52%。
我们付了召回的代价，却没真的隔离。
...
**L0 必须最先跑**，且 prompt 里不得出现规则库、契约、schema、任何清单 ——
它就是裸模型条件本身。**这条保证了本 Skill 的下限就是裸模型**：
后面各层只能在它之上做加法，或者带理由地驳回某一条，不能凭空让它变少。
...
**这一层必须第一个跑，且必须最干净。** 在它返回之前，不得让任何其他子会话碰论文。
...
旧的发现层同时干两件事：**做实质审阅** + **做测绘与路由**。
两件事合在一起时，测绘用的结构化要求会挤掉审阅的开放性。现在拆开：

- 实质审阅 → 上移为 L0，无约束，因此更强；
- 测绘与路由 → 下沉为 L0b，纯机械，因此更省。

**合规清单也不再需要单独的发现通道** —— M2/M4/M6 的规则库本身就是那些清单。
在发现层再抄一遍只会多一次压缩：实测双通道里的清单通道反而把
「全文无精确 P 值 / 检验统计量 / CI / 效应量」这类条目打包丢失，
而 M4 的统计规则库天然覆盖它。**清单归规则库，不归发现层。**
