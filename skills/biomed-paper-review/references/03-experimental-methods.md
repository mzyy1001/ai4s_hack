...
| 引用是否指向原始方法学文献而非综述？ | 离线层只能看引文题名线索，判不准就不判；X1 见 §11.1 | 同上（minor） |
...
| 已知误认细胞系 | **离线层不查**（需 Cellosaurus/ICLAC），见 §11.2 | —— |

**重要边界**：当前已实现的离线层只能查「有没有写」，查不了「写的对不对」。
不得因为某细胞系在你的记忆里可能有问题就报；权威库比对属于一期可选联网增强（§11.2），
connector 未运行或外部源不可得时不得产 mismatch。
