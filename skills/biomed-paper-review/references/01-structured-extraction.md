...
> **在分层架构中的位置（2026-08-07）**：M1 是 Layer 1 与 Layer 2 之间的抽取层。
> 它**不产 finding**，职责是把字段与**可核验标识符**（细胞系、UniProt accession、
> 人类基因符号、物种学名、RRID、化合物名、PDB 编号、NCT 注册号、数据登录号、
> 参考文献 DOI）抽出来，登记为 Layer 3 外部核验的查询入口。
> 没有这一步，X1 的主动扫描无从触发 —— 而主动扫描正是相对裸模型的主要增益来源。
> 抽取本身不需要读 M2–M7 的规则库。
