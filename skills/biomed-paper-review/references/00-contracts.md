...
#### 1.4.1 `evidence_refs[]` 必须**逐条特异**，不得霰弹式挂全集

`evidence_refs[]` 的含义是「**支撑这一条 finding 的那些证据**」，
不是「本次运行收集到的所有证据」。

**实测反例**（一次真实运行）：某篇论文的报告里，**每一条 finding 都挂了全部 24 条
证据引用**，包括与该条毫无关系的外部记录。形式上完全合规 —— 非空、每条都能解析 ——
但**可审计性归零**：读者无法回答「哪条外部记录支撑了『被引文献已撤稿』这个判断」。
对一个以可审计为卖点的工具，这比少写几条证据严重得多。

**规则**：

1. 只挂**直接支撑该条判断**的证据；能去掉而不影响该条成立的，一律去掉。
2. 一条 finding 引用**多条外部证据**时，必须在 `detail` 里说明每条各自证明了什么。
3. **禁止**把 `evidence_registry` 的键全集直接赋给某条 finding 的 `evidence_refs`。

**L5 自检（渲染前必做）**：统计各 finding 的 `evidence_refs` 集合，
若**超过一半的 finding 共享完全相同的引用列表**，判定为霰弹式引用，
必须逐条收窄后再渲染；无法收窄的条目，宁可只保留最直接的那一条，
也不要挂全集。
